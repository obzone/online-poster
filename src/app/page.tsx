...
export default async function Home({searchParams: {month, orgId}}: {searchParams: { [key: string]: string | undefined }}) {
  const queryMonth = month ? new Date(month+'-15' as string) : new Date()
  const [decoration, data] = await Promise.all([getMonthGlobalStyle(queryMonth), getAllActivities(queryMonth, orgId as string || '1')])
    ...
    <>
      <div className={styles.container}  >
        ...
        <div className={`${styles.month} customizeBG`} style={{...decoration?.style}} >
          <NavigationHeader defaultValue={orgId as string} month={queryMonth} />
          <WeekDayHeader />
          {
